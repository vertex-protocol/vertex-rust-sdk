...
use crate::engine;
use crate::engine::MaxLpMintableResponse;
use crate::utils::client_error::none_error;
use crate::utils::wrapped_option_utils::optional_bool_to_string;

vertex_builder!(
    MaxLpMintableBuilder,
    VertexQuery,
    subaccount: [u8; 32],
    product_id: u32,
    spot_leverage: bool;

    build_and_call!(self, query, get_max_lp_mintable => MaxLpMintableResponse);

    pub fn build(&self) -> Result<engine::Query> {
        fields_to_vars!(self, product_id, subaccount);
        Ok(engine::Query::MaxLpMintable {
            sender: subaccount,
            product_id,
